    ...
    try {
      ...
      if (!currentAuth) {
        ...
        if (!favorites.find(fav => fav.id === item.id)) {
          ...
          await AsyncStorage.setItem('@guest_favorites', JSON.stringify(favorites));
        }
      ...
      if (!users[userIndex].favorites.find(fav => fav.id === item.id)) {
        users[userIndex].favorites.push({ ...item, addedAt: new Date().toISOString() });
        await saveUsersDatabase(users);
      }
